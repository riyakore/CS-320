...
def compare(expected_csv, actual_csv):
    ...
    for qnum in sorted(expected_rows.keys()):
        ...
        if qnum == 19:
            print(f'your model scored {float(actual["value"])} ({float(expected["value"])} and above earns full credit on q19)')
        ...
        elif compare_fn(eval(expected["value"]), eval(actual["value"]), config):
            if qnum == 17: 
                passing = passing + 2.0
            else: 
                passing = passing + 1.0
    ...
    result["score"] = score
    result["summary"] = f"Result: {passing} of 22 points, for an estimated score of {score}% (prior to grader deductions)."
    return result
