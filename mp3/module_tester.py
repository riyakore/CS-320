...
def loans_test():
    ...
    # checking if exception is thrown for wrong bank names
    try:
        wrong_bank = loans.Bank("Invalid Bank")
    except ValueError as e:
        pass  
    loans_points += 1

    # checking invalid lei number
    newrez_bank = loans.Bank("NEWREZ LLC")
    assert newrez_bank.lei != "0"
    loans_points += 1

    # checking if the number of loans match with the bank
    assert len(newrez_bank) == 4340
...
def search_test():
    ...
    # testing with adding invalid keys
    try:
        t = search.BST()
        t.add("", )
    except TypeError:
